...
class SpecTask < RSpec::Core::RakeTask
  def initialize(name=:spec)
    super(name) do
      ...
      @rcov_opts = %w{--exclude ^rcov/,[^/]*\.gemspec,^spec/,^spec_help/,bundle-paths/ --sort coverage --threshold 101 -o doc/coverage --xrefs --no-color} 
      @rcov = true
  ...
  def spec_command
    @spec_command ||= 
      begin
        ...
        if rcov
          cmd_parts += [*rcov_path]
          cmd_parts += ["-Ispec_help#{File::PATH_SEPARATOR}lib", *rcov_opts]
